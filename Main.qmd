...
casedata <- data %>% filter(redcap_repeat_instrument == "case_abstraction")
...
#pivot long
screendata <- screendata %>% pivot_longer(cols = starts_with("screen_type"),
...
```{r}
#counts for 2024
data2024 <- data %>% filter(year == 2024)

tablescreen <- data2024 %>% filter(redcap_repeat_instrument == "screening_abstraction") %>% 
...
table2024 <- table2024 %>% mutate(percentage = (distinctscreens/screencount)*100) %>% select(-redcap_repeat_instrument)
